...
const _getInstructionIfApplicable = (
  instruction: string,
  condition: boolean
): string => (condition ? instruction : "");

const _trimAllValuesInObject = (
...
const getInstructions = (options: PromptOptions): string[] => {
  ...
  const instructions = [
    ...
    _getInstructionIfApplicable(
      `The goal of the text is to ${objective}.`,
      !!objective
    ),
    _getInstructionIfApplicable(
      `The tone of voice of the text should be ${tone}.`,
      !!tone
    ),
    _getInstructionIfApplicable(
      `The text should be generated for this target audience: "${audience}".`,
      !!audience
    ),
    _getInstructionIfApplicable(
      `Format and structure of the text should be ${format}.`,
      !!format
    ),
    _getInstructionIfApplicable(
      `The table should include the following tables: ${tableColumns
        ?.map((k) => `"${k}"`)
        .join(", ")}.`,
    ...
    _getInstructionIfApplicable(
      `Strictly consider this set of constraints: ${additionalConstraints
        ?.map((k) => `"${k}"`)
        .join(", ")}.`,
      !!additionalConstraints
    ),
    ...
    _getInstructionIfApplicable(
      `The generated text should be in the style of "${mimicPerson}".`,
      !!mimicPerson
    ),
    _getInstructionIfApplicable(
      `At the end, clearly request about "${callToAction}".`,
      !!callToAction
    ),
    _getInstructionIfApplicable(
    ...
    _getInstructionIfApplicable(
      `The text should be in ${language} language.`,
      !!language
    ),
  ];
